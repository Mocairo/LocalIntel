...
- 概览首页：默认只显示总体指标、运行状态、摘要、重点提醒等概览信息
- 左侧导航：在概览、今日情报、观察雷达、来源状态之间切换
- 日期、分类、状态、关键词筛选
- 今日主线和重点排序
- LLM 摘要与本地回退摘要
- 全球时事中文翻译
...
观察雷达从 `interests.toml` 的 `[[watchlist]]` 读取对象。每次更新情报后，系统会先用本地规则匹配当天条目，再让 LLM 判断是否有明显动向；结果写入 SQLite，页面刷新只读缓存。首页会展示当前观察结果和近 7 次走势，便于判断某个对象是持续升温还是偶发命中。点击观察对象可以打开详情抽屉，查看最近几次判断、动作建议、摘要和代表条目。
